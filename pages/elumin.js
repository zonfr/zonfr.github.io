<div id="elumin-page">
    <img id="logo-img" src="src\img\elumin_logo.png"></img> <img id="title-img" src="src\img\elumin_title.png"></img>
    <p class="desc-text">Elumin is an exploration game in an energy plant on an isolated isle, you&#x27;re a mechanical engineer, your job is to do maintenance all around the site. When you arrive the 14 people team you&#x27;re supposed to find is nowhere to be found. The energy production sector can&#x27;t be turned off due to an unknown cause. From now on you&#x27;re the only one in charge of keeping operations stable. In the mean time exploring the site will grant you answers on what happened and why.</p>
    <div class="screenshot-container">
        <img class="screenshot-img" src="src\img\elumin_banner.png"></img>
        <img class="screenshot-img" src="src\img\elumin_banner.png"></img>
        <img class="screenshot-img" src="src\img\elumin_banner.png"></img>
        <img class="screenshot-img" src="src\img\elumin_banner.png"></img>
    </div>
    <p><iframe src="https://store.steampowered.com/widget/2684580/" frameborder="0" class="widget-steam"></iframe></p>
</div>